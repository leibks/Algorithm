...
// Constant time.
public class RepeatedQuicksort {
	public static void main(String[] args) {
		...
		ArrayList<Request> prepare = new ArrayList<Request>();
		for (Request r : myRequests) {
			if (r.cancel) {
				//cancel this request
				for (int i = 0; i < prepare.size(); i++) {
					if(prepare.get(i).compareTo(r) == 0) {
						prepare.remove(i);
					}
				}
				...
				for (Request s : prepare) {
					//check if this request starting time is earlier than single time.
					//if not, that means requests after it cannot be chosen and we should
					//stop this loop, because now prepare is order 
					if(startS < s.getStartMinute()) {
						break;
					}
